...
class ProductController
{
	...
	): array
	{
		...
		foreach ($aWCProduct as $aItem) {
			...
			//nếu số lượng sp bằng 20 hoặc bằng số lượng limit thì thoát khỏi vòng lặp
	...
	public function getProducts(WP_REST_Request $oRequest)
	{
		...
		try {
			...
			//kiểm tra các product đã setting thủ công
			$aManual = $this->getManualBadges($aManualId);

			//mảng id  product cần kiểm tra automatic
			...
			//kiểm tra các product còn lại có được set automatic không
