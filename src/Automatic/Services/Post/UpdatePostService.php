...
class UpdatePostService extends PostService implements IService, IDeleteUpdateService
{
    ...
    public function performSaveData(): array
    {
        try {
            ...
            return MessageFactory::factory()->success(
                esc_html__('Congrats! The automatic has been updated successfully.', MYSHOPKIT_MB_WP_REST_NAMESPACE),
                [
                    'id' => $id
                ]
            );
    ...
    public function validateFields(): IService
    {
        if (empty($this->postID)) {
            throw new Exception(esc_html__('The ID is required.', MYSHOPKIT_MB_WP_REST_NAMESPACE));
        }

        $this->isPostAuthor($this->postID);
        $this->isPostType($this->postID, $this->aRawData['postType']);
