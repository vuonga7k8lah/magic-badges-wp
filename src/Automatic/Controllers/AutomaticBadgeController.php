...
{
	use TraitMainMiddleware, TraitIsPostAuthor, TraitUpdateInterval;
	...
	public function createAutomatic(WP_REST_Request $oRequest)
	{
		try {
			...
			//new arrival
			if ($postType == AutoPrefix::namePrefix('new_arrival') && !empty($interval)) {
	...
	public function detectBadge(array $aProduct, string $userID)
	{
		...
		try {
			...
			return MessageFactory::factory()->success(esc_html__('list data', MYSHOPKIT_MB_WP_REST_NAMESPACE),
